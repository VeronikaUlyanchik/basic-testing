// Uncomment the code below and write your tests
import { simpleCalculator, Action } from './index';

const testCases = [
  { a: 1, b: 2, action: Action.Add, expected: 3 },
  { a: 2, b: 2, action: Action.Add, expected: 4 },
  { a: 3, b: 2, action: Action.Add, expected: 5 },
  { a: 3, b: 2, action: Action.Subtract, expected: 1 },
  { a: 10, b: 6, action: Action.Subtract, expected: 4 },
  { a: 3, b: 2, action: Action.Multiply, expected: 6 },
  { a: 7, b: 2, action: Action.Multiply, expected: 14 },
  { a: 49, b: 7, action: Action.Divide, expected: 7 },
  { a: 50, b: 25, action: Action.Divide, expected: 2 },
  { a: 3, b: 2, action: Action.Exponentiate, expected: 9 },
  { a: 11, b: 2, action: Action.Exponentiate, expected: 121 },
  { a: 3, b: 2, action: 'delete', expected: null },
  { a: 3, b: 2, action: 'lol', expected: null },
  { a: '3', b: '2', action: Action.Exponentiate, expected: null },
  { a: '3ssss', b: '2', action: Action.Add, expected: null },
];

describe('simpleCalculator', () => {
  test.each(testCases)('.result($a, $b)', ({ a, b, action, expected }) => {
    expect(simpleCalculator({ a: a, b: b, action: action })).toBe(expected);
  });
  // Consider to use Jest table tests API to test all cases above
});
